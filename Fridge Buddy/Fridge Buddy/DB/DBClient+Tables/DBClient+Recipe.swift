...
extension DBClient {
  func createRecipeTable() throws {
    ...
    "CREATE TABLE IF NOT EXISTS \(tableName)(id TEXT PRIMARY KEY, name TEXT, yieldAmount INT, imageName TEXT);"
...
extension DBClient {
  func insertRecipe(_ recipe: Recipe) throws {
    ...
    "INSERT INTO Recipe (id, name, yieldAmount, imageName) VALUES (?, ?, ?, ?);"
    ...
    if sqlite3_prepare_v2(self.db, insertStatementString, -1, &insertStatement, nil) == SQLITE_OK {
      ...
      sqlite3_bind_text(insertStatement, 4, recipe.imageName.utf8, -1, nil)
  ...
  func updateRecipe(_ recipe: Recipe) throws {
    ...
    "UPDATE Recipe SET name = ?, yieldAmount = ?, imageName = ? WHERE id = ?;"
    ...
    if sqlite3_prepare_v2(self.db, updateStatementString, -1, &updateStatement, nil) == SQLITE_OK {
      ...
      sqlite3_bind_text(updateStatement, 3, recipe.imageName.utf8, -1, nil)
  ...
  func readRecipe(withDebugPrintOn: Bool = false) throws -> [Recipe] {
    let queryStatementString = "SELECT id, name, yieldAmount, imageName FROM Recipe;"
    ...
    if sqlite3_prepare_v2(self.db, queryStatementString, -1, &queryStatement, nil) == SQLITE_OK {
      ...
      while sqlite3_step(queryStatement) == SQLITE_ROW {
        ...
        let recipe = Recipe(id: UUID(uuidString: id)!, name: name, yieldAmount: yieldAmount, imageName: imageName)
